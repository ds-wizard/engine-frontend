module KMEditor.TagEditor.Models exposing
    ( Model
    , addQuestionTag
    , containsChanges
    , generateEvents
...
initialModel branchUuid =
    ...
    , questionTagsDict = Dict.empty
    , dirty = False
    , submitting = Unset
    }


setKnowledgeModel : Model -> KnowledgeModel -> Model
setKnowledgeModel model km =
    { model
        | knowledgeModel = Success km
...
generateEvents : Model -> Seed -> KnowledgeModel -> ( Seed, List Event )
generateEvents model seed knowledgeModel =
    let
        pathMap =
