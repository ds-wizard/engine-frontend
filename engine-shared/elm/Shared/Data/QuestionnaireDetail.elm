...
encode questionnaire =
    E.object
        [ ( "phaseUuid", Uuid.encode questionnaire.phaseUuid )
...
createQuestionnaireDetail package km =
    ...
    , phaseUuid = Maybe.unwrap Uuid.nil Uuid.fromUuidString (List.head km.phaseUuids)
    , selectedTagUuids = []
    , templateId = Nothing
    , template = Nothing
    , formatUuid = Nothing
...
evaluateQuestion appState questionnaire path question =
    let
        ...
        requiredNow =
            if questionnaire.phaseUuid == Uuid.nil then
                True

            else
                Question.isDesirable appState questionnaire.knowledgeModel.phaseUuids (Uuid.toString questionnaire.phaseUuid) question
