...
view appState model =
    Page.actionResultView appState (viewContent appState model) model.roles


viewContent : AppState -> Model -> List Role -> Html Msg
viewContent appState model roles =
    Container.simpleForm
        ...
        , Form.viewSimple
            ...
            , formView = formView appState roles model.form |> Html.map FormMsg
...
formView : AppState -> List Role -> Form FormError UserCreateForm -> Html Form.Msg
formView appState roles form =
    let
        roleOptions =
            roles |> List.sortBy .name |> List.map Role.toFormOption
    in
    div []
        ...
        , FormGroup.select appState.locale roleOptions form "roleUuid" <| gettext "Role" appState.locale
