...
viewProps templates =
    ...
    , guideLink = WizardGuideLinks.settingsDocumentSubmission
...
formView templates appState form =
    let
        ...
        servicesInput =
            if enabled then
                FormGroup.list appState.locale (serviceFormView appState templates) form "services" (gettext "Services" appState.locale) (gettext "Add service" appState.locale)
...
serviceFormView appState templates form i =
    ...
    div [ class "card bg-light mb-4" ]
        [ div [ class "card-body" ]
            [ div [ class "row" ]
                [ div [ class "col" ]
                    [ FormGroup.input appState.locale form idField (gettext "ID" appState.locale) ]
            ...
            , FormGroup.input appState.locale form nameField (gettext "Name" appState.locale)
            , FormGroup.markdownEditor appState.locale (WizardGuideLinks.markdownCheatsheet appState.guideLinks) form descriptionField (gettext "Description" appState.locale)
            , div [ class "input-table" ]
                ...
                , FormGroup.list appState.locale (supportedFormatFormView appState templates supportedFormatsField) form supportedFormatsField "" (gettext "Add format" appState.locale)
                ]
            , div [ class "input-table" ]
                ...
                , FormGroup.list appState.locale (propFormView propsField) form propsField "" (gettext "Add property" appState.locale)
                ]
...
requestFormView appState form prefix =
        ...
        multipartFileNameInput =
            if multipartEnabled then
                ...
                div [ class "nested-group" ]
                    [ FormGroup.input appState.locale form multipartFileName (gettext "Multipart File Name" appState.locale)
                    ]
    ...
    div []
        ...
        , div [ class "nested-group mt-2" ]
            [ FormGroup.select appState.locale HttpMethod.options form methodField (gettext "Method" appState.locale)
            , FormGroup.input appState.locale form urlField (gettext "URL" appState.locale)
            , FormGroup.list appState.locale (headerFormView appState headersField) form headersField (gettext "Headers" appState.locale) (gettext "Add header" appState.locale)
...
headerFormView appState prefix form index =
    let
        ...
        ( headerError, headerErrorClass ) =
            FormGroup.getErrors appState.locale headerField (gettext "Header" appState.locale)
