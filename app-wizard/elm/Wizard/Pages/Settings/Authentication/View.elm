...
formView openIDPrefabs appState form =
    let
                ...
                twoFactorInputs =
                    if twoFactorAuthEnabled then
                        ...
                        div [ class "nested-group" ]
                            [ formWrap <| FormGroup.input appState.locale form "twoFactorAuthCodeLength" (gettext "Code Length" appState.locale)
                            , formWrap <| FormGroup.input appState.locale form "twoFactorAuthExpiration" (gettext "Expiration" appState.locale)
        ...
        externalAuthentication =
            ...
            , FormGroup.listWithCustomMsg appState.locale formMsg (serviceFormView appState openIDPrefabs) form "services" (gettext "OpenID Services" appState.locale) (gettext "Add service" appState.locale)
            ]
    in
    div [ class "Authentication" ]
        ([ mapFormMsg <| FormGroup.select appState.locale (Role.options appState) form "defaultRole" (gettext "Default role" appState.locale)
...
serviceFormView appState openIDPrefabs form i =
    ...
    div [ class "card bg-light mb-4" ]
        [ div [ class "card-body" ]
            ...
            , div [ class "row" ]
                [ div [ class "col" ]
                    [ mapFormMsg <| FormGroup.input appState.locale form idField (gettext "ID" appState.locale) ]
            ...
            , div [ class "row" ]
                [ div [ class "col" ] [ mapFormMsg <| FormGroup.input appState.locale form clientIdField (gettext "Client ID" appState.locale) ]
                , div [ class "col" ] [ mapFormMsg <| FormGroup.secret appState.locale form clientSecretField (gettext "Client Secret" appState.locale) ]
                ]
            , mapFormMsg <| FormGroup.input appState.locale form urlField (gettext "URL" appState.locale)
            , div [ class "input-table", dataCy "settings_authentication_service_parameters" ]
                ...
                , mapFormMsg <| FormGroup.list appState.locale (serviceParameterView appState parametersField) form parametersField "" (gettext "Add parameter" appState.locale)
                ]
            , hr [] []
            , div [ class "row" ]
                [ div [ class "col-7" ]
                    [ div [ class "row" ]
                        [ div [ class "col" ]
                            [ mapFormMsg <| FormGroup.inputAttrs [ placeholder ExternalLoginButton.defaultIcon ] appState.locale form styleIconField (gettext "Icon" appState.locale)
                            ]
                        , div [ class "col" ]
                            [ mapFormMsg <| FormGroup.input appState.locale form nameField (gettext "Name" appState.locale)
                            ]
                        ]
                    , div [ class "row" ]
                        [ div [ class "col" ]
                            [ mapFormMsg <| FormGroup.inputAttrs [ placeholder ExternalLoginButton.defaultBackground ] appState.locale form styleBackgroundField (gettext "Background Color" appState.locale)
                            ]
                        , div [ class "col" ]
                            [ mapFormMsg <| FormGroup.inputAttrs [ placeholder ExternalLoginButton.defaultColor ] appState.locale form styleColorField (gettext "Text Color" appState.locale)
                            ]
                        ]
                    ]
