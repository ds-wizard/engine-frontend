module Wizard.Components.Questionnaire2.Components.FileUploadModal exposing
    ( FileConfig
    , Model
    , Msg
    , UpdateConfig
...
view appState isKmEditor model =
    Lazy.lazy4 viewLazy appState.locale appState.maxUploadFileSize isKmEditor model


viewLazy : Gettext.Locale -> Int -> Bool -> Model -> Html Msg
viewLazy locale maxUploadFileSize isKmEditor model =
    let
        invalidFileSelection =
            case model.file of
                Just file ->
                    not (isValidFileType model.fileConfig file) || not (isValidFileSize maxUploadFileSize model.fileConfig file)
        ...
        submitButtonTooltip =
            if isKmEditor && not invalidFileSelection then
                tooltip (gettext "File upload is not available in the knowledge model editor" locale)
        ...
        cancelButton =
            button
                ...
                [ text (gettext "Cancel" locale) ]

        content =
            case model.file of
                Just file ->
                    contentFileView locale maxUploadFileSize model file

                Nothing ->
                    contentDropzoneView locale model

        modalContent =
            [ div [ class "modal-header" ]
                [ h5 [ class "modal-title" ] [ text (gettext "Upload a file" locale) ] ]
...
contentFileView : Gettext.Locale -> Int -> Model -> File -> Html Msg
contentFileView locale maxUploadFileSize model file =
    let
        fileTypeError =
            if isValidFileType model.fileConfig file then
                ...
                div [ class "alert alert-danger" ]
                    [ Markdown.toHtml []
                        (String.format
                            (gettext "**The file type is not allowed for this question.**\n\nChoose one of the following: %s" locale)
                            [ fileTypes ]
                        )
                    ]

        fileSizeError =
            if isValidFileSize maxUploadFileSize model.fileConfig file then
                Html.nothing
                ...
                div [ class "alert alert-danger" ]
                    [ Markdown.toHtml []
                        (String.format
                            (gettext "The file cannot be larger than %s." locale)
                            [ ByteUnits.toReadable (getFileMaxSize maxUploadFileSize model.fileConfig) ]
                        )
                    ]
    in
    div []
...
contentDropzoneView : Gettext.Locale -> Model -> Html Msg
contentDropzoneView locale model =
    div
        [ class "dropzone"
        ...
        [ button [ onClick Pick, class "btn btn-secondary" ] [ text (gettext "Choose file" locale) ]
        , p [] [ text (gettext "Or just drop it here" locale) ]
        ]
...
isValidFileSize : Int -> FileConfig -> File -> Bool
isValidFileSize maxUploadFileSize fileConfig file =
    File.size file <= getFileMaxSize maxUploadFileSize fileConfig


getFileMaxSize : Int -> FileConfig -> Int
getFileMaxSize maxUploadFileSize fileConfig =
    Maybe.unwrap maxUploadFileSize (min maxUploadFileSize) fileConfig.maxSize
