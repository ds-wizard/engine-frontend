module Wizard.Components.Listing.View exposing
    ( CustomFilterConfig
    , Filter(..)
    , SimpleFilterConfig
...
import Wizard.Components.Listing.Models exposing (Item, Model)
import Wizard.Components.Listing.Msgs exposing (Msg(..))
import Wizard.Components.ListingDropdown as ListingDropdown exposing (ListingDropdownItem)
    ...
    case config.updated of
        Just updated ->
            ...
            span (tooltip readableTime)
                [ text <| String.format (gettext "Updated %s" appState.locale) [ inWordsWithConfig { withAffix = True } (locale appState.locale) time updated.currentTime ] ]
