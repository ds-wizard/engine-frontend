module Wizard.Components.Questionnaire.FeedbackModal exposing
    ( Model
    , Msg(..)
    , init
    , update
...
import Wizard.Components.Questionnaire.FeedbackForm as FeedbackForm exposing (FeedbackForm)
...
feedbackModalContent appState model =
    ...
    , feedbackList
    , FormGroup.input appState.locale model.feedbackForm "title" (gettext "Title" appState.locale) |> Html.map FeedbackFormMsg
    , FormGroup.textarea appState.locale model.feedbackForm "content" (gettext "Description" appState.locale) |> Html.map FeedbackFormMsg
    ]
