...
viewEventDetailSetReply appState cfg data question =
    let
        replyView ( icon, replyText ) =
            li [] [ span [ class "fa-li" ] [ icon ], text replyText ]
    ...
    let
        ...
        fold event acc =
            ...
            if QuestionnaireEvent.isInvisible event then
                ...
                case QuestionnaireEvent.getQuestionUuid event of
                    Just questionUuid ->
                        if not (QuestionnaireDetail.isVersion questionnaire event) && Maybe.unwrap False ((==) createdBy) (Dict.get questionUuid acc.questions) then
                            acc

                        else
                            { questions = Dict.insert (Maybe.withDefault "" mbQuestionUuid) createdBy acc.questions
