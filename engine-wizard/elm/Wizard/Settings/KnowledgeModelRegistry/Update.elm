...
update wrapMsg msg appState model =
    case msg of
        ...
        ToggleRegistrySignup isOpen ->
            ( { model | registrySignupOpen = isOpen }, Cmd.none )
...
handleForm wrapMsg formMsg appState model =
    case ( formMsg, Form.getOutput model.registrySignupForm ) of
        ( Form.Submit, Just form ) ->
            ...
            in
            ( { model | registrySigningUp = Loading }, cmd )

        _ ->
            ( { model
                | registrySignupForm = Form.update RegistrySignupForm.validation formMsg model.registrySignupForm
              }
            , Cmd.none
            )
...
handlePostSignupComplete appState model result =
    let
        ( newResult, cmd ) =
            case result of
                Ok _ ->
                    ( Success (lg "apiSuccess.registry.signup" appState)
                    , Cmd.none
                    )

                Err error ->
                    ( ApiError.toActionResult (lg "apiError.registry.signup.postError" appState) error
                    , getResultCmd result
                    )
    in
    ( { model | registrySigningUp = newResult }, cmd )
