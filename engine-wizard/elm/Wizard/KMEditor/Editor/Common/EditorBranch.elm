...
filterDeleted editorBranch =
    List.filter (not << flip List.member editorBranch.deletedUuids)
...
applyDelete { entityUuid, parentUuid } editorBranch =
    let
        newEditorBranch =
            if editorBranch.activeUuid == entityUuid then
                setActiveEditor (Just parentUuid) editorBranch

            else
                editorBranch
    in
    setDeleted entityUuid newEditorBranch
