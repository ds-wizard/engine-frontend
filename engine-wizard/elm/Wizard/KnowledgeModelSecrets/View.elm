module Wizard.KnowledgeModelSecrets.View exposing (view)

import Form
import Gettext exposing (gettext)
import Html exposing (Html, button, code, div, em, p, span, table, tbody, td, text, th, thead, tr)
import Html.Attributes exposing (attribute, class, colspan, width)
...
import Wizard.KnowledgeModelSecrets.Models exposing (Model)
import Wizard.KnowledgeModelSecrets.Msgs exposing (Msg(..))
    ...
    table [ class "table table-hover border rounded" ]
        [ thead [ class "table-light" ]
            [ tr []
                ...
                , th [] [ text (gettext "Last updated" appState.locale) ]
                , th [] []
                ]
            ]
    ...
    tr []
        ...
        , td [ class "align-middle" ]
            [ em (class "text-secondary text-nowrap pe-2" :: tooltip readableTime)
                [ text (inWordsWithConfig { withAffix = True } (TimeDistance.locale appState) secret.updatedAt appState.currentTime)
                ]
            ]
...
viewCreateSecretModal appState model =
    let
        modalContent =
            [ Html.map CreateFormMsg <| FormGroup.input appState model.createSecretForm "name" (gettext "Name" appState.locale)
            ...
            , Html.map CreateFormMsg <| FormGroup.secret appState model.createSecretForm "value" (gettext "Value" appState.locale)
            ]
...
viewEditSecretModal appState model =
    let
        modalContent =
            [ Html.map EditFormMsg <| FormGroup.input appState model.editSecretForm "name" (gettext "Name" appState.locale)
            , FormExtra.mdAfter (gettext "Knowledge Model Secret Name can only contain alphanumeric characters, hyphens, underscores, and dots." appState.locale)
            , Html.map EditFormMsg <| FormGroup.secret appState model.editSecretForm "value" (gettext "Value" appState.locale)
            ]
